...
def get_vehicle_parameters(track_type):
    ...
    """
    # Base vehicle parameters (typical FSAE values)
    vehicle_params = {
        'mass': 617.4,  # lbs (280 kg)
        'cg_height': 0.88,  # ft (0.267 m)
        'wheelbase': 5.09,  # ft (1.55 m)
        'track_width': 4.0,  # ft (1.22 m)
        'tire_mu': 1.8,  # peak friction coefficient
        'aero_cl': 2.5,  # downforce coefficient
        'aero_cd': 1.1,  # drag coefficient
    ...
    if track_type == 'endurance':
        ...
        vehicle_params.update({
            'base_speed': 75,  # mph
            'max_speed': 112,  # mph (50 m/s)
            'min_speed': 17,   # mph (7.5 m/s)
            'speed_factor': 1.0,
        ...
        vehicle_params.update({
            ...
            'min_speed': 25,   # mph
            'speed_factor': 0.7,
            'corner_factor': 1.2  # more aggressive cornering
        })
...
def calculate_max_cornering_speeds(curvatures, vehicle_params):
    ...
    for i in range(n_points):
        if curvatures[i] > 0:
            ...
            max_cornering_speeds[i] = min(max_corner_speed_mph, vehicle_params['max_speed'])
        else:
            # Straight section - limited by power/aero
            max_cornering_speeds[i] = vehicle_params['max_speed']
