...
Physic    # Track-specific parameters
    if track_type == 'endurance':
        # Endurance: higher speeds, longer straights, gentler corners
        max_speed_mph = vehicle_config['max_velocity'] * 2.23694  # m/s to mph (~112 mph)
        min_speed_mph = max(25, vehicle_config['min_velocity'] * 2.23694)  # m/s to mph, min 25 mph
        vehicle_params.update({
            'base_speed': 60,  # mph - starting speed
            'max_speed': max_speed_mph,
            'min_speed': min_speed_mph,
            'speed_factor': 1.0,
            'corner_factor': 0.9  # slightly conservative cornering
        })
    else:  # autocross
        # Autocross: lower speeds, tight corners, more technical  
        max_speed_mph = min(85, vehicle_config['max_velocity'] * 2.23694 * 0.75)  # Scale down for autocross
        min_speed_mph = max(20, vehicle_config['min_velocity'] * 2.23694)  # m/s to mph, min 20 mph
        vehicle_params.update({
            'base_speed': 40,  # mph - starting speed
            'max_speed': max_speed_mph,
            'min_speed': min_speed_mph,
            'speed_factor': 0.8,
            'corner_factor': 1.1  # more aggressive cornering for tight autocross
        })ion following MATLAB Lap_Sim methodology.
...
# Add python directory to path to import vehicle_config
...
def calculate_realistic_velocities(x_coords, y_coords, track_type):
    ...
    print(f"  Curvature stats - Min: {np.min(curvatures):.6f}, Max: {np.max(curvatures):.6f}, Mean: {np.mean(curvatures):.6f}")
    print(f"  Non-zero curvatures: {np.sum(curvatures > 0.001)}/{len(curvatures)}")
    ...
    # Debug: Print cornering speed statistics
...
def get_vehicle_parameters(track_type):
    """
    Get vehicle parameters from vehicle_config.py based on track type.
    ...
    """
    # Get base vehicle configuration
    vehicle_config = get_vehicle_config()
    
    # Convert to imperial units and physics format
    mass_lbs = vehicle_config['mass'] * 2.20462  # kg to lbs
    wheelbase_ft = vehicle_config['wheelbase'] * 3.28084  # m to ft
    track_width_ft = vehicle_config['track_width'] * 3.28084  # m to ft
    cg_height_ft = vehicle_config['cg_height'] * 3.28084  # m to ft
    
    # Base vehicle parameters from vehicle_config
    vehicle_params = {
        'mass': mass_lbs,
        'cg_height': cg_height_ft,
        'wheelbase': wheelbase_ft,
        'track_width': track_width_ft,
        'tire_mu': vehicle_config['tire_params']['mu'],  # peak friction coefficient
        'aero_cl': vehicle_config['downforce_coefficient'],  # downforce coefficient
        'aero_cd': vehicle_config['drag_coefficient'],  # drag coefficient
        'max_lat_accel': 1.8,  # max lateral g's (tire limited)
        'max_lng_accel': 1.2,  # max longitudinal acceleration (power limited)
        'max_lng_decel': 2.0,  # max braking deceleration
    }
    ...
    if track_type == 'endurance':
        # Endurance: higher speeds, longer straights, gentler corners
        vehicle_params.update({
            'base_speed': 60,  # mph - more conservative starting speed
            'max_speed': vehicle_config['max_velocity'] * 2.23694,  # m/s to mph, no artificial cap
            'min_speed': vehicle_config['min_velocity'] * 2.23694,  # m/s to mph
            'speed_factor': 1.0,
            'corner_factor': 0.8  # less aggressive cornering
        })
    else:  # autocross
        # Autocross: lower speeds, tight corners, more technical
        vehicle_params.update({
            'base_speed': 40,  # mph - more conservative starting speed
            'max_speed': vehicle_config['max_velocity'] * 2.23694,  # m/s to mph, no artificial cap
            'min_speed': vehicle_config['min_velocity'] * 2.23694,  # m/s to mph
            'speed_factor': 0.8,  # slightly reduced for autocross
            'corner_factor': 1.0  # standard cornering
        })
...
def calculate_max_cornering_speeds(curvatures, vehicle_params):
    ...
    for i in range(n_points):
        if curvatures[i] > 0:
            ...
            # Don't artificially cap at max_speed - let physics determine speed
            # Only limit to prevent unreasonably high speeds on gentle curves
            max_reasonable_speed = vehicle_params['max_speed'] * 1.2  # Allow 20% over for gentle curves
            max_cornering_speeds[i] = min(max_corner_speed_mph, max_reasonable_speed)
        else:
            # Straight section - limited by power/aero, but can exceed normal max
            max_cornering_speeds[i] = vehicle_params['max_speed'] * 1.1  # Allow slight overspeed on straights
...
def apply_acceleration_limits(x_coords, y_coords, max_cornering_speeds, vehicle_params):
    ...
    # Initialize first point with speed appropriate for first corner
    velocities[0] = min(max_cornering_speeds[0], vehicle_params['base_speed'])
    ...
    for i in range(1, n_points):
        ...
        if ds > 0:
            ...
            # Apply acceleration limit (simplified power model)
            max_accel = vehicle_params['max_lng_accel'] * 32.2  # ft/s²
            
            # Account for drag
            drag_force = vehicle_params['aero_cd'] * v_prev**2
            drag_accel = drag_force / vehicle_params['mass'] * 32.2
            net_accel = max_accel - drag_accel
            
            # Kinematic equation: v² = v₀² + 2as
            v_max_sq = v_prev**2 + 2 * max(net_accel, 0) * ds
            v_accel_limit = np.sqrt(max(v_max_sq, v_prev**2)) * 3600 / 5280  # mph
            
            # Take minimum of acceleration and cornering limits
            velocities[i] = min(v_accel_limit, v_corner_limit, vehicle_params['max_speed'])
        else:
...
def finalize_velocity_profile(velocities, vehicle_params):
    ...
    velocities = np.clip(velocities, vehicle_params['min_speed'], vehicle_params['max_speed'])
    ...
    # If changes are too extreme, apply additional smoothing
    if max_change > 15:  # more than 15 mph change between adjacent points
        velocities = gaussian_filter1d(velocities, sigma=2.0)
        velocities = np.clip(velocities, vehicle_params['min_speed'], vehicle_params['max_speed'])
